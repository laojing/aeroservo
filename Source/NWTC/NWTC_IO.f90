...
!=======================================================================
   ...
   IF ( ErrStat /= 0 ) THEN
      ...
      IF ( ALLOCATED(Ary) ) THEN ! or Sttus=151 on IVF
         ErrMsg = 'Error allocating memory for the '//TRIM( Descr )//' array; array was already allocated.'
      ELSE
         ErrMsg = 'Error '//TRIM(Num2LStr(AryDim1*AryDim2*AryDim3))//' '//TRIM( Descr )//' array.'
      END IF
   ELSE
      ErrStat = ErrID_None
      ErrMsg  = ''
...
!=======================================================================
   ...
   IF ( ErrStat /= 0 ) THEN
      ...
      IF ( ALLOCATED(Ary) ) THEN ! or Sttus=151 on IVF
         ErrMsg = 'Error allocating memory for the '//TRIM( Descr )//' array; array was already allocated.'
      ELSE
         ErrMsg = 'Error allocating '//TRIM(Num2LStr(AryDim1*AryDim2*BYTES_IN_INT))//' '//TRIM( Descr )//' array.'
      END IF
   ELSE
      ErrStat = ErrID_None
      ErrMsg  = ''
...
!=======================================================================
   ...
   IF ( ErrStat /= 0 ) THEN
      ...
      IF ( ALLOCATED(Ary) ) THEN ! or Sttus=151 on IVF
         ErrMsg = 'Error allocating memory for the '//TRIM( Descr )//' array; array was already allocated.'
      ELSE
         ErrMsg = 'Error allocat of memory for the '//TRIM( Descr )//' array.'
      END IF
   ELSE
      ErrStat = ErrID_None
      ErrMsg  = ''
...
!=======================================================================
   ...
   IF ( IOS /= 0 )  THEN
      Failed = .TRUE.
      ErrStat = ErrID_Fatal
      ErrMsg = 'OpenFUnkFile:Cannot open file "'//TRIM( OutFile )//'".'
   ...
   SUBROUTINE ParseInclInfo ( InclInfo, FileName, RangeBeg, RangeEnd, ErrStat, ErrMsg )
      ...
      IF ( TRIM( Words(2) ) == 'IN' )  THEN
         ...
         IF ( DashLoc > 0 )  THEN                                             ! Must be in the form of "<num1>-<num2>".
            ...
            IF ( ErrStatLcl /= 0 )  THEN
               CALL SetErrStat(ErrID_Fatal,' line range.',ErrStat,ErrMsg,RoutineName)
               RETURN
            ENDIF ! ( ErrStatLcl /= 0 )
            ...
            IF ( RangeBeg <= 0 )  THEN
               CALL SetErrStat(ErrID_Fatal,' line range.'//NewLine// &
...
!=======================================================================
   SUBROUTINE ProcessComFile ( TopFileName, FileInfo, ErrStat, ErrMsg )
      ...
      ALLOCATE ( FileInfo%FileList( FileInfo%NumFiles ) , STAT=ErrStatLcl )
         IF ( ErrStatLcl /= 0 )  THEN
            CALL SetErrStat( ErrID_Fatal, 'ray.' , ErrStat, ErrMsg, RoutineName )
            CALL Cleanup()
            RETURN
         ENDIF
