...
ENDIF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FileLine)>0) THEN
        OutData%FileLine = UNPACK( IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(OutData%FileLine))-1 ), mask1, 0_IntKi )
      END IF
  ...
  ELSE
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FileIndx)>0) THEN
          OutData%FileIndx = UNPACK( IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(OutData%FileIndx))-1 ), mask1, 0_IntKi )
      END IF
